...
function lotsToUsdPip(lots: number): number {
  return Math.round(lots * 10 * 10000) / 10000;
}
  ...
  return (
    <div style={{ background: "#0a0a0f", border: "1px solid #1e1e2e", borderRadius: 10, padding: "10px 14px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 3 }}>
        <span style={{ fontSize: 9, color: "#333", letterSpacing: 2 }}>{label}</span>
        {live && (
          <div className="dot-wrap" style={{ width: 10, height: 10 }}>
            <div className="dot-ping" />
            <div className="dot-core" style={{ width: 5, height: 5 }} />
          </div>
        )}
      </div>
      <span style={{ fontSize: 14, color: "#555", fontWeight: 700, fontFamily: "Courier New, monospace" }}>{value}</span>
    </div>
  );
}
...
export default function App() {
  ...
  const [draftLots, setDraftLots] = useState(String(config.lots));
      ...
      <div className="content" style={{ width: cardW }}>
        ...
        <div className="card-animate" style={{ ...cardStyle, animationDelay: "0.1s" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginBottom: editing ? 18 : 0 }}>
            ...
            <button
              className="edit-btn"
              onClick={() => { setEditing(!editing); setDraftLots(String(config.lots)); }}
          ...
          {editing && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <InfoRow label="PAR" value={config.pair} />
                <InfoRow label="EUR RATE" value={formatNum(config.usdEur, 4)} live={rateStatus === "live"} />
              </div>

              <div>
                <div style={{ fontSize: 10, color: "#f0b429", marginBottom: 6, letterSpacing: 2 }}>LOTES</div>
                <input
                  ...
                  min="0.01"
                  value={draftLots}
                  onChange={(e) => setDraftLots(e.target.value)}
                  style={{ width: "100%", background: "#0a0a0f", border: "1px solid #f0b42944", borderRadius: 8, color: "#fff", padding: "10px 14px", fontSize: 18, fontFamily: "Courier New, monospace", boxSizing: "border-box" }}
                />
              </div>

              <div style={{ background: "#0a0a0f", border: "1px solid #1e1e2e", borderRadius: 10, padding: "12px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 10, color: "#444", letterSpacing: 2 }}>USD POR PIP (CALCULADO)</span>
                <span style={{ fontSize: 16, color: "#f0b429", fontWeight: 700, fontFamily: "Courier New, monospace" }}>
                  ${formatNum(lotsToUsdPip(Number(draftLots) || config.lots), 4)}
                </span>
              </div>
              ...
              <button
                className="save-btn"
                onClick={() => {
                  const lots = Number(draftLots) || config.lots;
                  const pipValue = lotsToUsdPip(lots);
                  setConfig((c) => ({ ...c, lots, pipValue }));
                  setEditing(false);
                }}
        ...
        <div style={{ textAlign: "center", marginTop: 14, fontSize: 11, color: "#2a2a35", letterSpacing: 4, fontStyle: "italic" }}>
          No Risk No Ferrari
        </div>
