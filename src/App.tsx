...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0f", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Courier New, monospace", padding: 20 }}>
      <style>{PULSE_STYLE}</style>
      <div style={{ width: "100%", maxWidth: 420 }}>

        <div style={{ textAlign: "center", marginBottom: 18 }}>
          <div style={{ fontSize: 11, letterSpacing: 6, color: "#f0b429", marginBottom: 6, textTransform: "uppercase" }}>Trading Tool</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: "#fff", letterSpacing: 2 }}>PIPSWINNERS CALCULATOR</div>
          <div style={{ fontSize: 12, color: "#555", marginTop: 4, letterSpacing: 2 }}>USD - EUR - PIPS</div>
        </div>

        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 16, minHeight: 22 }}>
          {rateStatus === "loading" && <span style={{ fontSize: 10, color: "#444", letterSpacing: 2 }}>OBTENIENDO TASA EN VIVO...</span>}
          {rateStatus === "live" && (
            <>
              <div className="dot-wrap"><div className="dot-ping" /><div className="dot-core" /></div>
              <span style={{ fontSize: 10, color: "#22c55e", letterSpacing: 2 }}>USD/EUR EN VIVO - {rateDate}</span>
            </>
          )}
          {rateStatus === "fallback" && <span style={{ fontSize: 10, color: "#f0b429", letterSpacing: 2 }}>TASA MANUAL - EDITA EN CONFIG</span>}
        </div>
        ...
        <div style={{ background: "#12121a", border: "1px solid #222", borderRadius: 12, padding: "16px 20px", marginBottom: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: editing ? 16 : 0 }}>
            ...
            <button onClick={() => { setEditing(!editing); setDraft(config); }} style={{ background: "transparent", border: "1px solid #f0b429", color: "#f0b429", borderRadius: 6, padding: "4px 12px", fontSize: 11, cursor: "pointer", letterSpacing: 1 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              {fields.map((f) => (
                <div key={f.key}>
                  ...
                  <input type={f.type} value={String(draft[f.key])} onChange={(e) => setDraft((d) => ({ ...d, [f.key]: e.target.value }))}
                    style={{ width: "100%", background: "#0a0a0f", border: "1px solid #333", borderRadius: 6, color: "#fff", padding: "8px 10px", fontSize: 13, fontFamily: "Courier New, monospace", boxSizing: "border-box" }} />
                </div>
              ))}
              <div style={{ gridColumn: "1/-1" }}>
                <button onClick={() => { setConfig({ pair: String(draft.pair), lots: Number(draft.lots), pipValue: Number(draft.pipValue), usdEur: Number(draft.usdEur) }); setEditing(false); }}
                  style={{ width: "100%", background: "#f0b429", border: "none", borderRadius: 8, color: "#0a0a0f", padding: 10, fontSize: 13, fontWeight: 700, cursor: "pointer", letterSpacing: 2 }}>
                  GUARDAR
                </button>
              </div>
            </div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {[["pips", "PIPS a $"], ["amount", "$ a PIPS"]].map(([key, label]) => (
            <button key={key} onClick={() => setActiveTab(key)}
              style={{ flex: 1, background: activeTab === key ? "#f0b429" : "#12121a", border: activeTab === key ? "none" : "1px solid #222", borderRadius: 8, color: activeTab === key ? "#0a0a0f" : "#555", padding: 10, fontSize: 12, fontWeight: 700, cursor: "pointer", letterSpacing: 2 }}>
              {label}
            </button>
          ))}
        </div>
        ...
        {activeTab === "pips" && (
          <div style={{ background: "#12121a", border: "1px solid #222", borderRadius: 12, padding: 20 }}>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
              <input type="number" placeholder="ej: 150 o -80" value={pips} onChange={(e) => setPips(e.target.value)}
                style={{ flex: 1, background: "#0a0a0f", border: "1px solid #333", borderRadius: 8, color: "#fff", padding: "12px 14px", fontSize: 20, fontFamily: "Courier New, monospace" }} />
              <button onClick={() => setPips("")} style={{ background: "#1a1a25", border: "1px solid #333", borderRadius: 8, color: "#555", padding: "0 14px", fontSize: 18, cursor: "pointer" }}>X</button>
            </div>
            {pipResult && (
              <div style={{ display: "grid", gap: 10 }}>
                <ResultBox label="USD" value={(pipResult.positive ? "+" : "") + "$" + formatNum(pipResult.usd)} positive={pipResult.positive} />
                <ResultBox label="EUR" value={(pipResult.positive ? "+" : "") + "E" + formatNum(pipResult.eur)} positive={pipResult.positive} />
                <div style={{ background: "#0a0a0f", borderRadius: 8, padding: "10px 14px", fontSize: 11, color: "#444", letterSpacing: 1 }}>
                  {Math.abs(Number(pips))} pips x ${config.pipValue}/pip = ${formatNum(Math.abs(pipResult.usd))} = E{formatNum(Math.abs(pipResult.eur))} @ {formatNum(config.usdEur, 4)}
                </div>
              </div>
            )}
          </div>
              ...
              <div style={{ display: "flex", gap: 4 }}>
                {["USD", "EUR"].map((c) => (
                  <button key={c} onClick={() => setCurrency(c)}
                    style={{ background: currency === c ? "#f0b429" : "#0a0a0f", border: currency === c ? "none" : "1px solid #333", borderRadius: 6, color: currency === c ? "#0a0a0f" : "#555", padding: "0 14px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                    {c === "USD" ? "$ USD" : "E EUR"}
                  </button>
                ))}
              </div>
              <input type="number" placeholder={currency === "USD" ? "ej: 30" : "ej: 27"} value={amount} onChange={(e) => setAmount(e.target.value)}
                style={{ flex: 1, background: "#0a0a0f", border: "1px solid #333", borderRadius: 8, color: "#fff", padding: "12px 14px", fontSize: 20, fontFamily: "Courier New, monospace" }} />
              <button onClick={() => setAmount("")} style={{ background: "#1a1a25", border: "1px solid #333", borderRadius: 8, color: "#555", padding: "0 14px", fontSize: 18, cursor: "pointer" }}>X</button>
            </div>
            {amountResult && (
              <div style={{ display: "grid", gap: 10 }}>
                <ResultBox label="PIPS" value={formatNum(amountResult.pips, 1) + " pips"} positive={true} gold />
                <div style={{ background: "#0a0a0f", borderRadius: 8, padding: "10px 14px", fontSize: 11, color: "#444", letterSpacing: 1 }}>
                  {currency === "EUR" ? "E" + amount + " / " + formatNum(config.usdEur, 4) + " = $" + formatNum(Number(amount) / config.usdEur) + " / $" + config.pipValue + "/pip" : "$" + amount + " / $" + config.pipValue + "/pip"} = {formatNum(amountResult.pips, 1)} pips
                </div>
              </div>
            )}
          </div>
        ...
        <a href="https://t.me/PIPSWINNERSCOMMUNITY" target="_blank" rel="noopener noreferrer"
          style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, marginTop: 16, padding: "14px 20px", background: "#0088cc11", border: "1px solid #0088cc44", borderRadius: 12, textDecoration: "none", cursor: "pointer" }}>
          ...
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#0088cc", letterSpacing: 3 }}>UNETE A LA COMUNIDAD</div>
